...
// 								homebase like '%$cari%'");
// } else {
    $sql = "select * from matkul a JOIN krs b on (a.idmatkul = b.idMatkul) JOIN dosen c ON (c.npp = b.nppDos) JOIN mhs d ON (d.nim = b.nim)";
...
?>
<div class="utama">
	<h2 class="text-center">Daftar KRS</h2>
	...
	if (mysqli_num_rows($hasil) == 0){
		...
		while($row=mysqli_fetch_assoc($hasil)){
			...
			<tr>
				...
				<td>
				<a class="btn btn-outline-primary btn-sm" href="editKrs.php?idkrs=<?php echo encryptid($row["idKrs"])?>">Edit</a>
				<a class="btn btn-outline-danger btn-sm" href="hpsKrs.php?idkrs=<?php echo encryptid($row["idKrs"])?>" id="linkHps" onclick="return confirm('Yakin dihapus nih?')">Hapus</a>
				</td>
			</tr>
			<?php 
			$no++;
